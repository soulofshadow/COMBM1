...
def build_model(w,n,width,height,data):
    ...
    rotation = [Bool("ro_%s" % i) for i in range(n)]
    ...
    domain_c = []
    for i in range(n):
        domain_c.append(And(Xposition[i] + width_r[i] <= w,
                            Xposition[i] >= 0, 
                            Yposition[i] >= 0,
                            Yposition[i] + height_r[i] <= h))

    #cumulative
    cumulative_x = cumulative(Xposition,width_r,height_r,h)
    cumulative_y = cumulative(Yposition,height_r,width_r,w)
    ...
    for i in range(n):
        for j in range(n):
            if i < j:
                nooverlap_c.append(Or(Xposition[i] + width_r[i] <= Xposition[j],
                                      Xposition[i] >= Xposition[j] + width_r[j],
                                      Yposition[i] + height_r[i] <= Yposition[j],
                                      Yposition[i] >= Yposition[j] + height_r[j]))
    ...
    for i in range(n):
        nogap_c.append(And(
                Or(Xposition[i] == 0, 
                Or([Xposition[i] == Xposition[j] + width_r[j]
                    for j in range(n)])),
                Or(Yposition[i] == 0,
                Or([Yposition[i] == Yposition[j] + height_r[j]
                    for j in range(n)]))))
    ...
    return Xposition, Yposition, h, rotation, constraints
...
def plot_result(r):
    ...
    for i in range(r['n']):
        if r['r'][i]:
            rect = plt.Rectangle((r['x'][i],r['y'][i]),r['height'][i],r['width'][i])
        else:
...
def write_file(r):
    ...
    with open(writepath + 'out-{0}.txt'.format(r['i']),'w') as f:  
        ...
        for i in range(r['n']):
            f.writelines('%d %d %d %d %s\n' % (r['width'][i],r['height'][i],r['x'][i],r['y'][i],r['r'][i]))
        f.close()
...
def run(instance):
    ...
    x, y, h, rotation, constraints = build_model(w,n,width,height,data)
    s.add(constraints)
    ...
    for i in range(data['minHeight'], data['maxHeight'] + 1):
        ...
        if s.check() == sat:
            ...
            return {'r':r_sol, 'i':instance, 'w':w, 'n':n, 'width':width, 'height':height, 'x':x_sol, 'y':y_sol, 'h':h_sol, 'duration':str(duration)}
        else:
            # duration = datetime.datetime.now() - start
            # print(duration)
