...
for file in files:
    ...
    if result.statistics['solutions']== 0:
        ...
        with open('./cp/output/inputOrder_random_geo_normal/out-{0}.txt'.format(fileIndex),'w') as f:  
            f.writelines('%d %d \n' %(w,result["objective"]))
            f.writelines(str(n)+'\n')
            for i in range(n):
        ...
        plt.plot()
        plt.savefig('./cp/output/inputOrder_random_geo_normal/pic-{0}.jpg'.format(fileIndex))
        plt.close(fig)
        #plt.show()
...
runingData.to_csv('./cp/output/inputOrder_random_geo_normal/result.csv') 
